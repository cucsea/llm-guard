...
DEFAULT_AGENT_IDENTITY = (
    "You are Jizhi, an intelligent AI assistant. "
...
def _build_snapshot_entry(
    ...
    always_active = bool(frontmatter.get("always_active", False))
    body = ""
    if always_active:
        try:
...
def build_skills_system_prompt(
    ...
    # Include the filesystem manifest so add/delete/modify of any skill file
    # invalidates the cache — the snapshot is validated against this same
    # manifest, but the LRU cache is in-memory and wouldn't otherwise know
    # that skill files changed between process-restarts.
    _manifest = _build_skills_manifest(skills_dir)
    _manifest_sig = hash(tuple(sorted((k, tuple(v)) for k, v in _manifest.items()))) if _manifest else 0
    cache_key = (
        str(skills_dir),
    ...
    skills_by_category: dict[str, list[tuple[str, str, dict]]] = {}
    category_descriptions: dict[str, str] = {}
    ...
    if snapshot is not None:
        ...
        for entry in snapshot.get("skills", []):
            ...
            if frontmatter_name in disabled or skill_name in disabled:
                continue
            if not entry.get("always_active") and not _skill_should_show(
            ...
            skills_by_category.setdefault(category, []).append(
                (frontmatter_name, entry.get("description", ""), entry)
            )
        ...
        for skill_file in iter_skill_index_files(skills_dir, "SKILL.md"):
            ...
            skills_by_category.setdefault(entry["category"], []).append(
                (entry["frontmatter_name"], entry["description"], entry)
            )
    ...
    for cat_skills in skills_by_category.values():
        for name, _desc, _entry in cat_skills:
    ...
    for ext_dir in external_dirs:
        ...
        for skill_file in iter_skill_index_files(ext_dir, "SKILL.md"):
            try:
                ...
                skills_by_category.setdefault(entry["category"], []).append(
                    (frontmatter_name, entry["description"], entry)
                )
            except Exception as e:
    ...
    if not skills_by_category:
        ...
        index_lines = []
        always_active_skills: list[dict] = []
        for category in sorted(skills_by_category.keys()):
            # Deduplicate and sort skills within each category
            seen = set()
            if category in demoted:
                names = sorted({name for name, _, _ in skills_by_category[category]})
            ...
            for name, desc, entry in sorted(skills_by_category[category], key=lambda x: x[0]):
                if name in seen:
                    continue
                seen.add(name)
                if entry.get("always_active"):
                    always_active_skills.append(entry)
                    if desc:
                        index_lines.append(f"    - {name} (always active): {desc}")
                    else:
                        index_lines.append(f"    - {name} (always active)")
                else:
                    if desc:
        ...
        if always_active_skills:
            result += "\n\n## Always-active skills (automatically loaded)\n"
            result += "The following skills are ALWAYS in effect. Follow their instructions without needing to call skill_view().\n"
            for entry in always_active_skills:
