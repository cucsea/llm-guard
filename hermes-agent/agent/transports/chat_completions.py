...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def _build_kwargs_from_profile(self, profile, model, sanitized, tools, params):
        ...
        import logging as _eb_log
        if "/plan/" in str(params.get("base_url", "")).lower():
            _eb_log.warning("ARK PLAN DEBUG: extra_body=%s has_input=%s has_input_content=%s",
                           extra_body,
                           "input" in extra_body if extra_body else False,
                           "input" in extra_body and isinstance(extra_body.get("input"), dict) and "content" in extra_body["input"] if extra_body else False)
            if extra_body and "input" in extra_body:
                _eb_log.warning("ARK PLAN DEBUG: input=%s", extra_body["input"])
