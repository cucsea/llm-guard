...
def _try_resolve_from_custom_pool(
    ...
    try:
        ...
        if not pool_api_key:
            return None
        _pool_api_mode = api_mode_override
        if _pool_api_mode:
            _pool_api_mode = _resolve_plain_custom_api_mode({"api_mode": _pool_api_mode}, base_url)
        else:
            _pool_api_mode = _detect_api_mode_for_url(base_url) or "chat_completions"
        return {
            "provider": provider_label,
            "api_mode": _pool_api_mode,
            "base_url": base_url,
...
def _resolve_named_custom_runtime(
    *,
    ...
    result = {
        "provider": "custom",
        "api_mode": _resolve_plain_custom_api_mode(custom_provider, base_url),
        "base_url": base_url,
