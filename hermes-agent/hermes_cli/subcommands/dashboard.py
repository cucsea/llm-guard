...
def build_dashboard_parser(
    ...
    # =========================================================================
    # home command — the lightweight management UI (web-home/)
    #
    # Same backend gateway as `dashboard`, but mounts the web-home SPA (a
    # light, tech-styled console: chat / sessions / models / skills / plugins /
    # config, with zh/en switching) instead of the full dashboard bundle.
    # cmd_dashboard sees `home_ui=True` and points HERMES_WEB_DIST at
    # web-home/dist before the build gate and the web_server import.
    # =========================================================================
    home_parser = subparsers.add_parser(
        "home",
        help="Start the lightweight Jizhi management console (web-home UI)",
        description=(
            "Launch the lightweight Jizhi management console — a light, "
            "tech-styled web UI for chat, sessions, models, skills, plugins "
            "and config, with Chinese/English switching. Shares the same "
            "backend server as `dashboard`."
        ),
    )
    _add_server_runtime_args(home_parser)
    home_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    home_parser.set_defaults(func=cmd_dashboard, home_ui=True, port=8801)
