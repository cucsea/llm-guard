    ...
    *,
    ...
    if root is not None and (not text or text in {".", "/"}):
        ...
        if not candidate.is_absolute():
            ...
            base = root if root is not None else policy.default_path
...
# =========================================================================
# Jizhi Home — lightweight SSE chat endpoint
#
# The full dashboard chat runs over a JSON-RPC WebSocket that drives the TUI
# gateway (tui_gateway/ws.py + server.py). The Home console (web-home/) wants
# a much simpler, dependency-free client: POST a message, read a Server-Sent
# Events stream back. This endpoint bridges the *same* agent runtime
# (tui_gateway.server._make_agent → AIAgent.run_conversation) to a plain SSE
# stream, so Home gets real streaming without the PTY/WS machinery.
#
# Design (mirrors gateway/platforms/api_server.py's SSE bridge):
#   - Build + run the (synchronous, blocking) agent in a worker thread.
#   - The agent's callbacks fire on that worker thread, so they hand events to
#     the event loop via loop.call_soon_threadsafe(queue.put_nowait, ...).
#   - The async generator drains the queue and yields SSE frames.
#   - We OVERRIDE the agent's callbacks after construction (rather than reuse
#     tui_gateway._agent_cbs, which routes through _emit → a WS transport we
#     don't have; unrouted _emit falls through to stdout). This keeps all
#     output on our queue.
#   - Persistence is automatic: the agent is built with session_db + session_id,
#     so run_conversation flushes user+assistant+tool messages to state.db.
# =========================================================================
# ── Clarify (interactive user input) state ────────────────────────────
# The clarify tool blocks the agent thread until the user responds via
# POST /api/home/chat/clarify/resolve. Module-level so the resolve
# endpoint and home_chat_stream share the same dict.
# =========================================================================
_home_clarify_lock: threading.RLock = threading.RLock()
_home_clarify_entries: Dict[str, "_HomeClarifyEntry"] = {}


@dataclass
class _HomeClarifyEntry:
    clarify_id: str
    question: str
...
class HomeChatRequest(BaseModel):
    message: str
    # Continue an existing conversation; omit/empty to start a new one.
...
@app.post("/api/home/chat/stream")
async def home_chat_stream(body: HomeChatRequest, request: Request):
    """Stream an agent conversation turn as Server-Sent Events.

    Events emitted (``event:`` name → ``data:`` JSON):
      - ``session``   {session_id}            — resolved/created session id
      - ``delta``     {text}                  — assistant text increment
      - ``reasoning`` {text}                  — reasoning/thinking increment
      - ``tool``      {phase, name, preview}  — tool start/complete
      - ``done``      {session_id, text, status, usage}
      - ``error``     {message}
    """
    import asyncio
    ...
    # New conversations get a fresh id; Home passes it back on later turns.
    session_id = (body.session_id or "").strip() or _uuid.uuid4().hex

    loop = asyncio.get_running_loop()
    queue: "asyncio.Queue" = asyncio.Queue()

    def _enqueue(event: str, payload: dict) -> None:
        # Callbacks fire on the agent worker thread; hop back to the loop.
        try:
    ...
    def _clarify_callback(question: str, choices: Optional[List[str]] = None) -> str:
        """Blocking callback invoked by the clarify tool on the agent thread."""
        clarify_id = _uuid.uuid4().hex
        entry = _HomeClarifyEntry(
            clarify_id=clarify_id,
            question=question,
            choices=list(choices) if choices else None,
        )
        with _home_clarify_lock:
            _home_clarify_entries[clarify_id] = entry
        _enqueue("clarify", {
            "clarify_id": clarify_id,
            "question": question,
            "choices": list(choices) if choices else None,
        })
        # Wait for the frontend to respond (max 10 minutes).
        entry.event.wait(timeout=600)
        with _home_clarify_lock:
            _home_clarify_entries.pop(clarify_id, None)
        return entry.response or ""

    def _run_agent_turn() -> None:
        """Blocking: build the agent, load history, run one turn."""
        try:
            from tui_gateway.server import _make_agent

            db = _open_session_db_for_profile(profile)
            try:
                # Resolve/continue an existing session; a brand-new id resolves
                # to itself (agent creates the row on first message flush).
                ...
                _enqueue("session", {"session_id": resolved})

                # Build the agent bound to this session + db so run_conversation
                # persists the turn automatically. sid/key are internal ids.
                agent = _make_agent(
                    sid=resolved,
                    key=resolved,
                    session_id=resolved,
                ...
                # Override callbacks to route into our SSE queue instead of the
                # WS _emit transport (which we don't have here).
                def _stream(delta):
                    if delta:
                        _enqueue("delta", {"text": delta})

                def _reasoning(text):
                    if text:
                        _enqueue("reasoning", {"text": text})

                def _tool_start(tc_id, name, args):
                    _enqueue("tool", {"phase": "start", "name": name})
                ...
                def _tool_complete(tc_id, name, args, result):
                    ...
                    _enqueue(
                        "tool",
                        {"phase": "complete", "name": name, "preview": preview},
                    )

                agent.reasoning_callback = _reasoning
                agent.thinking_callback = _reasoning
                agent.tool_start_callback = _tool_start
                agent.tool_complete_callback = _tool_complete
                agent.clarify_callback = _clarify_callback
                # NOTE: do NOT also set agent.stream_delta_callback here.
                # _fire_stream_delta() feeds BOTH agent.stream_delta_callback
                # AND agent._stream_callback (the run_conversation stream_callback=
                # param below). Setting both would emit every token twice.
                # Neutralize the remaining WS-oriented callbacks so nothing
                # leaks to stdout via the unrouted _emit fallback.
                for _attr in (
                ...
                try:
                    import inspect
                    if "task_id" in inspect.signature(
                        agent.run_conversation
                    ).parameters:
                        run_kwargs["task_id"] = resolved
                except (TypeError, ValueError):
                    pass
                ...
                if attachments:
                    ...
                    for att in attachments:
                        try:
                            att_path = att.path
                            if att.type == "image":
                                mime = att.mime_type or mimetypes.guess_type(att.name)[0] or "image/png"
                                with open(att_path, "rb") as _f:
                                    _data = _b64.b64encode(_f.read()).decode("ascii")
                                content_parts.append({
                                    "type": "image_url",
                                    "image_url": {"url": f"data:{mime};base64,{_data}"}
                                })
                            else:
                                content_parts.append({
                                    "type": "text",
                                    "text": f"[Attached file: {att.name}]({att_path})"
                                })
                        except Exception as exc:
                            _log.warning("Skipping attachment %s: %s", att.name, exc)
                    user_message = content_parts
                else:
                    user_message = message

                result = agent.run_conversation(user_message, **run_kwargs)

                final_text = ""
                status = "complete"
                usage = {}
                effective_sid = resolved
                if isinstance(result, dict):
                    final_text = result.get("final_response", "") or ""
                    effective_sid = result.get("session_id", resolved) or resolved
            ...
            _log.exception("home chat stream failed")
            _enqueue("error", {"message": str(exc)})
        finally:
            _enqueue("__end__", {})

    async def _generator():
        # Kick off the blocking agent turn on a worker thread.
        task = asyncio.create_task(run_in_threadpool(_run_agent_turn))
        try:
            while True:
                try:
...
@app.post("/api/home/chat/clarify/resolve")
async def home_clarify_resolve(body: ClarifyResolveRequest):
    """Resolve a pending clarify request from the home chat UI.

    Called by the frontend when the user picks a choice or submits text
    in response to a ``clarify`` SSE event. Unblocks the agent thread.
    """
    entry = _home_clarify_entries.get(body.clarify_id)
    if entry is None:
        raise HTTPException(status_code=404, detail="Clarify request not found or already expired")
...
def _resolve_chat_argv(
    ...
    # If TERMINAL_CWD isn't set by the config bridge (terminal.cwd:
    # "auto"/absent), default it to the project root so the TUI agent's
    # Terminal tool always starts from a known-good directory.  Without this,
    # the tool falls through to os.getcwd() of the TUI process which may
    # resolve to a non-existent or MSYS-unmappable path on Windows, causing
    # ``builtin cd ... || exit 126`` in every bash invocation.
    env.setdefault("TERMINAL_CWD", str(PROJECT_ROOT.parent))
    ...
    # When the PTY child cannot attach to the in-memory gateway (profile-scoped
    # chats, or a failed attach), ``gatewayClient`` spawns its own
    # ``python -m tui_gateway.entry``.  Its ``resolvePython`` only probes
    # ``.venv``/``venv`` dirs and otherwise falls back to a bare ``python`` —
    # which on Windows resolves to the WindowsApps Store stub and dies with
    # exit code 9009 ("gateway exited (9009)").  Pin the interpreter to the one
    # currently running the dashboard (the embedded/venv Python that can import
    # ``tui_gateway``), mirroring ``hermes_cli.main._launch_tui``. ``setdefault``
    # so an explicit operator override still wins.
    from hermes_cli.main import PROJECT_ROOT as _PROJECT_ROOT
    env.setdefault("HERMES_PYTHON", sys.executable)
    env.setdefault(
        "HERMES_PYTHON_SRC_ROOT",
        os.environ.get("HERMES_PYTHON_SRC_ROOT", str(_PROJECT_ROOT)),
    )
