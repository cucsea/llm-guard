...
try:
    ...
    _setup_logging(
        mode=(
            ...
            in {"dashboard", "home", "serve", "gui", "desktop"}
            else "cli"
        )
    )
except Exception:
...
def _find_stale_dashboard_pids(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    patterns = [
        ...
        # `hermes home` is the same long-lived server mounting the web-home
        # SPA. Reap/stop/report it alongside `dashboard` and `serve`.
        "hermes home",
        "hermes_cli.main home",
        "hermes_cli/main.py home",
    ]
    self_pid = os.getpid()
...
def cmd_dashboard(args):
    ...
    # `home` is the lightweight management UI (web-home/): the SAME backend
    # server as `dashboard`, but it mounts the web-home SPA instead of the
    # full dashboard bundle. We select it purely by pointing HERMES_WEB_DIST
    # at the pre-built web-home dist BEFORE the build gate (12218) and before
    # web_server is imported (its WEB_DIST is read at import). Setting it in
    # os.environ here also means any profile re-exec child inherits it via
    # env = os.environ.copy(), so it serves the same SPA even though the child
    # is spawned as `dashboard`. An explicit HERMES_WEB_DIST from the caller
    # always wins.
    if getattr(args, "home_ui", False) and "HERMES_WEB_DIST" not in os.environ:
        os.environ["HERMES_WEB_DIST"] = str(PROJECT_ROOT / "web-home" / "dist")
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        ...
        "computer-use",
        "config", "console", "cron", "curator", "dashboard", "home", "serve", "debug", "doctor",
