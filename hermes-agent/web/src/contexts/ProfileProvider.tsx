  ...
  useEffect(() => {
    ...
    Promise.all([api.getProfiles(), api.getActiveProfile()])
      .then(([profilesRes, info]) => {
        ...
        // Skip switch when active=="default" (no explicit active_profile file)
        // and current differs (e.g. custom HERMES_HOME deployment) — "default"
        // would resolve to ~/.hermes/ instead of the actual workspace root,
        // causing model/chart pages to show "(unknown)".
        if (urlProfile === null && active !== current && active !== "default") {
