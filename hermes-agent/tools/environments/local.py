...
def _find_bash() -> str:
    ...
    # Check for PortableGit relative to HERMES_HOME / JIZHI_HOME (workspace
    # root).  The user may place a portable Git for Windows alongside their
    # project so the Terminal tool finds a MSYS2 bash without requiring a
    # system-wide Git installation.
    _workspace_portable_dir = None
    for _env_key in ("HERMES_HOME", "JIZHI_HOME"):
        _val = os.environ.get(_env_key, "").strip()
        if _val:
            _candidate = os.path.join(_val, "PortableGit", "bin", "bash.exe")
            if os.path.isfile(_candidate):
                _workspace_portable_dir = _candidate
                break
    if _workspace_portable_dir:
        candidates.append(_workspace_portable_dir)
...
def _bash_starts(bash: str) -> bool:
    ...
    # On Windows, also verify this is a MSYS2/Git-Bash (not WSL bash).
    # WSL bash uses /mnt/d/ for Windows drives, not the /d/ format that
    # MSYS2 uses. All our builtin cd commands use /d/ (MSYS2) form, so
    # WSL bash would fail with "No such file or directory".
    # We detect MSYS2 by checking for ``cygpath``, a binary unique to
    # MSYS2/Cygwin that is present in all Git for Windows distributions
    # (PortableGit, MinGit, full installer).
    if ok and _IS_WINDOWS:
        try:
            probe = subprocess.run(
                [bash, "--noprofile", "--norc", "-c", "command -v cygpath 2>/dev/null || exit 1"],
                capture_output=True,
                text=True,
                timeout=15,
                creationflags=windows_hide_flags() if _IS_WINDOWS else 0,
            ...
            if probe.returncode != 0:
                logger.debug(
                    "bash %s is not MSYS2/Git-Bash (no cygpath); rejecting",
                    bash,
                )
                ok = False
        except Exception as exc:
            logger.debug("cygpath probe error for %s: %s", bash, exc)
            ok = False
