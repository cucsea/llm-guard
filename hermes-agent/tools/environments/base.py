...
class BaseEnvironment(ABC):
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # ``|| true`` (not ``|| exit 126``) so MSYS2/Git Bash on Windows does
        # not abort the entire script when it can't translate the cwd (e.g.
        # D: drive not mounted in the MSYS2 mount table — issue #42750).
        # Falls back to $HOME, then current dir.
        parts.append(
            f"builtin cd -- {quoted_cwd} 2>/dev/null "
            f"|| builtin cd -- \"$HOME\" 2>/dev/null "
            f"|| true"
        )
